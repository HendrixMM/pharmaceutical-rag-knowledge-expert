...
class TestBaselineMetadataPresence:
    ...
    def test_all_benchmarks_have_baselines(self, benchmark_files):
        ...
        for file_path in benchmark_files:
            ...
            assert "baselines" in data["metadata"], \
                f"{file_path.name}: Missing 'metadata.baselines' key"
    ...
    def test_baselines_has_all_subsections(self, benchmark_files):
        ...
        for file_path in benchmark_files:
            ...
            assert "cloud" in baselines, \
                f"{file_path.name}: Missing 'cloud' in baselines"
            assert "self_hosted" in baselines, \
                f"{file_path.name}: Missing 'self_hosted' in baselines"
...
class TestCloudBaselineStructure:
    ...
    def test_cloud_baseline_required_fields(self, benchmark_files):
        ...
        for file_path in benchmark_files:
            ...
            for field in required_fields:
                assert field in cloud_baseline, \
                    f"{file_path.name}: Missing '{field}' in cloud baseline"
    ...
    def test_cloud_baseline_field_types(self, benchmark_files):
        ...
        for file_path in benchmark_files:
            ...
            assert isinstance(cloud["average_latency_ms"], (int, float)), \
                f"{file_path.name}: average_latency_ms should be numeric"
            assert isinstance(cloud["success_rate"], (int, float)), \
                f"{file_path.name}: success_rate should be numeric"
            assert isinstance(cloud["average_cost_per_query"], (int, float)), \
                f"{file_path.name}: average_cost_per_query should be numeric"
            assert isinstance(cloud["average_accuracy"], (int, float)), \
                f"{file_path.name}: average_accuracy should be numeric"
            assert isinstance(cloud["notes"], str), \
                f"{file_path.name}: notes should be string"
    ...
    def test_cloud_baseline_value_ranges(self, benchmark_files):
        ...
        for file_path in benchmark_files:
            ...
            assert cloud["average_latency_ms"] > 0, \
                f"{file_path.name}: average_latency_ms should be positive"

            # Success rate between 0 and 1
            assert 0 <= cloud["success_rate"] <= 1, \
                f"{file_path.name}: success_rate should be between 0 and 1"

            # Accuracy between 0 and 1
            assert 0 <= cloud["average_accuracy"] <= 1, \
                f"{file_path.name}: average_accuracy should be between 0 and 1"

            # Cost should be positive (cloud has cost)
            assert cloud["average_cost_per_query"] > 0, \
                f"{file_path.name}: cloud average_cost_per_query should be positive"

            # Notes should not be empty
            assert len(cloud["notes"]) > 0, \
                f"{file_path.name}: notes should not be empty"
...
class TestSelfHostedBaselineStructure:
    ...
    def test_self_hosted_baseline_required_fields(self, benchmark_files):
        ...
        for file_path in benchmark_files:
            ...
            for field in required_fields:
                assert field in self_hosted, \
                    f"{file_path.name}: Missing '{field}' in self_hosted baseline"
    ...
    def test_self_hosted_baseline_field_types(self, benchmark_files):
        ...
        for file_path in benchmark_files:
            ...
            assert isinstance(self_hosted["average_latency_ms"], (int, float)), \
                f"{file_path.name}: average_latency_ms should be numeric"
            assert isinstance(self_hosted["success_rate"], (int, float)), \
                f"{file_path.name}: success_rate should be numeric"
            assert isinstance(self_hosted["average_cost_per_query"], (int, float)), \
                f"{file_path.name}: average_cost_per_query should be numeric"
            assert isinstance(self_hosted["average_accuracy"], (int, float)), \
                f"{file_path.name}: average_accuracy should be numeric"
            assert isinstance(self_hosted["notes"], str), \
                f"{file_path.name}: notes should be string"
    ...
    def test_self_hosted_baseline_zero_cost(self, benchmark_files):
        ...
        for file_path in benchmark_files:
            ...
            assert self_hosted["average_cost_per_query"] == 0.0, \
...
class TestRegressionThresholds:
        ...
        for file_path in benchmark_files:
            ...
            for field in required_fields:
                assert field in thresholds, \
                    f"{file_path.name}: Missing '{field}' in regression_thresholds"
    ...
    def test_regression_thresholds_types(self, benchmark_files):
        ...
        for file_path in benchmark_files:
            ...
            assert isinstance(thresholds["accuracy_drop_percent"], (int, float)), \
                f"{file_path.name}: accuracy_drop_percent should be numeric"
            assert isinstance(thresholds["cost_increase_percent"], (int, float)), \
                f"{file_path.name}: cost_increase_percent should be numeric"
            assert isinstance(thresholds["latency_increase_percent"], (int, float)), \
...
class TestCloudVsSelfHostedComparison:
    ...
    def test_cloud_vs_self_hosted_cost_difference(self, benchmark_files):
        ...
        for file_path in benchmark_files:
            ...
            assert cloud_cost > 0, \
                f"{file_path.name}: cloud cost should be > 0"
            assert self_hosted_cost == 0.0, \
                f"{file_path.name}: self_hosted cost should be 0.0"
    ...
    def test_cloud_faster_than_self_hosted(self, benchmark_files):
        ...
        for file_path in benchmark_files:
            ...
            assert cloud_latency < self_hosted_latency, \
...
class TestBaselineMetadataCompleteness:
    ...
    def test_baseline_metadata_for_each_category(self, benchmark_files):
        ...
        for file_path in benchmark_files:
            ...
            assert "baselines" in data["metadata"], \
                f"{file_path.name}: Missing baselines"
            assert "cloud" in data["metadata"]["baselines"], \
                f"{file_path.name}: Missing cloud baseline"
            assert "self_hosted" in data["metadata"]["baselines"], \
                f"{file_path.name}: Missing self_hosted baseline"
            assert "regression_thresholds" in data["metadata"]["baselines"], \
        ...
        for expected_category in expected_categories:
            assert expected_category in found_categories, \
                f"Missing category: {expected_category}"
    ...
    def test_notes_field_populated(self, benchmark_files):
        ...
        for file_path in benchmark_files:
            ...
            assert len(cloud_notes) >= 10, \
                f"{file_path.name}: cloud notes too short"
            assert len(self_hosted_notes) >= 10, \
                f"{file_path.name}: self_hosted notes too short"

            # Notes should contain relevant keywords
            assert any(keyword in cloud_notes.lower() for keyword in ["cloud", "nvidia", "build", "api"]), \
                f"{file_path.name}: cloud notes should mention cloud/API context"
            assert any(keyword in self_hosted_notes.lower() for keyword in ["local", "nim", "self", "container", "gpu"]), \

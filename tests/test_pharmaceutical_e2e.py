...
class TestBenchmarkExecutionE2E:
    ...
    def test_full_benchmark_run_simulated(self, sample_benchmark_data):
        ...
        assert result["metadata"]["total_queries"] == expected_queries, \
            f"Should process {expected_queries} queries"
        ...
        assert 0 <= result["metrics"]["average_accuracy"] <= 1, \
            "Accuracy should be between 0 and 1"
        assert 0 <= result["metrics"]["average_overall_score"] <= 1, \
            "Overall score should be between 0 and 1"
        assert result["metrics"]["average_latency_ms"] >= 0, \
            "Latency should be non-negative"
        assert result["metrics"]["average_credits_per_query"] >= 0, \
            "Credits should be non-negative"

        # Check query results
        assert "query_results" in result
        assert len(result["query_results"]) == expected_queries, \
            f"Should have {expected_queries} query results"
        ...
        for query_result in result["query_results"]:
            ...
            assert has_scores or has_error, \
                "Query result should have either scores or error"
    ...
    def test_full_benchmark_run_handles_query_failures(self):
        ...
        assert result["metadata"]["failed_queries"] == 1, \
            "Should have exactly one failed query"
        assert result["metadata"]["successful_queries"] == 1, \
            "Should have exactly one successful query"
        assert result["metadata"]["total_queries"] == 2, \
            "Should have 2 total queries"
...
class TestCLIExecutionE2E:
    ...
    def test_cli_execution_simulation_single_category(self, capsys):
        ...
        with patch('sys.argv', test_args), \
             patch('scripts.run_pharmaceutical_benchmarks.BenchmarkRunner') as MockRunner:
    ...
    def test_cli_execution_all_benchmarks(self):
        ...
        with patch('sys.argv', test_args), \
            ...
            assert mock_runner_instance.run_benchmark.call_count == 2, \
                "Should run 2 benchmarks"
...
class TestRegressionDetectionWorkflowE2E:
    ...
    def test_regression_detection_workflow_with_regressions(self):
        ...
        assert "accuracy_regression" in flags, \
            "8.2% accuracy drop should trigger regression"
        assert "cost_regression" in flags, \
            "30% cost increase should trigger regression"
        ...
        assert pk_comparison["current"]["accuracy"] > pk_comparison["baseline"]["accuracy"], \
            "Accuracy should improve"
        assert pk_comparison["current"]["cost"] < pk_comparison["baseline"]["cost"], \
            "Cost should improve"
        assert pk_comparison["current"]["latency"] < pk_comparison["baseline"]["latency"], \
            "Latency should improve"

        pk_flags = pk_comparison["regression_flags"]
        assert len(pk_flags) == 0, \
            f"Should have no regressions for improved metrics, got {pk_flags}"
    ...
    def test_regression_detection_workflow_missing_categories(self):
        ...
        assert "drug_interactions" in comparison, \
            "Should compare matching category"

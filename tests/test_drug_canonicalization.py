...
def test_drug_canonicalization():
    ...
    result = ddi_processor.analyze_drug_interactions(
        test_papers, "Coumadin", ["Warfarin"]  # brand name input  # generic name
    )
    ...
    result2 = ddi_processor.analyze_drug_interactions(
        test_papers, "warfarin", ["coumadin"]  # generic name input  # brand name
    )
    ...
    assert result.get("analyzed_papers") == result2.get(
        "analyzed_papers"
    ...
    try:
        ...
        assert (
            "error" not in none_result or none_result.get("analyzed_papers") == 1
...
def test_interaction_detection():
    ...
    result = ddi_processor.analyze_drug_interactions(interaction_papers, "warfarin", ["fluconazole"])

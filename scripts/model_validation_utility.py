...
class NVIDIAModelValidator:
    ...
    ENDPOINTS = [
        ...
        EndpointTest(
            ...
            endpoint_path="/embeddings",
            headers_template={"Content-Type": "application/json"},
            test_payload={"model": "nvidia/nv-embed-v1", "input": ["Test pharmaceutical research query"]},
        ),
    ]
    ...
    def test_endpoint_access(self, endpoint: EndpointTest) -> TestResult:
        ...
        headers = {"Authorization": f"{endpoint.expected_auth_header} {self.api_key}", **endpoint.headers_template}
    ...
    def test_model_on_endpoint(self, model: ModelTest, endpoint_base: str, endpoint_path: str) -> TestResult:
        ...
        if model.endpoint_type == "embedding":
            ...
            payload = {"model": model.full_name, "messages": model.test_input, "max_tokens": 50, "temperature": 0.1}
    ...
    def discover_working_endpoints(self) -> List[Tuple[str, TestResult]]:
        ...
        for endpoint in self.ENDPOINTS:
            ...
            if result.success:
                ...
                print(
                    f"  ❌ {endpoint.name}: {result.status_code} - {result.error_message[:100] if result.error_message else 'Connection failed'}"
                )
        ...
        for model in self.MODELS_TO_TEST:
            ...
            for endpoint_name, base_url, path in endpoints_to_test:
                ...
                if result.success:
                    ...
                    print(
                        f"    ❌ {endpoint_name}: {result.status_code} - {result.error_message[:50] if result.error_message else 'Failed'}"
                    )
...
def main():
    ...
    if args.test_all_endpoints:
        validator.discover_working_endpoints()
    ...
    print(
        f"Working Endpoints: {', '.join(analysis['accessible_endpoints']) if analysis['accessible_endpoints'] else 'None'}"
    )

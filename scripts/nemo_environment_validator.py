...
class NeMoEnvironmentValidator:
    ...
    def validate_python_environment(self) -> None:
        ...
        if in_venv:
            ...
            self.results.append(
                ValidationResult(
                    component="Virtual Environment", status="PASS", message=f"Virtual environment active: {venv_path}"
                )
            )
        else:
            self.results.append(
    ...
    def validate_nvidia_api_access(self) -> None:
        ...
        test_payload = {"input": ["test connection"], "model": "nvidia/nv-embedqa-e5-v5"}

        try:
            response = requests.post(self.nemo_endpoints["embedding"], headers=headers, json=test_payload, timeout=10)
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                for line in result.stdout.strip().split("\n"):
                    parts = line.split(", ")
                    if len(parts) >= 3:
                        gpu_info.append({"name": parts[0], "memory_mb": int(parts[1]), "driver_version": parts[2]})
    ...
    def print_report(self, report: Dict[str, Any]) -> None:
        ...
        status_emoji = {"READY": "🟢", "CAUTION": "🟡", "BLOCKED": "🔴"}
